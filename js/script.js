...
// 현재 전시 부분 슬라이드
const bannerSlide = document.querySelector('#bannerSliderWraper');
const bannerSlideItems = document.querySelectorAll('#bannerSliderWraper li');

    console.log(bannerSlide);
    console.log(bannerSlideItems);
    console.dir(bannerSlideItems[0]);


    let index = 0;
    const timer = setInterval(function() {
      if(index < bannerSlideItems.length - 1) {
        console.log(index);
        bannerSlideItems[index].style.marginRight = '-79vw';
        index += 1;
      } else {
        index = 0;
        bannerSlideItems[index].style.marginRight = '0vw';
        for(let i = 0; i < bannerSlideItems.length; i++) {
          bannerSlideItems[i].style.marginRight = '0vw';
        }
        console.log('done');
...
const displayMove = document.getElementById('displayWraper');

displayMove.addEventListener('mouseover',()=>{
  displayMove.style.left = '-40vw';
});
displayMove.addEventListener('mouseout',()=>{
  displayMove.style.left = '33vw';
});
